...
{ public void Configure(EntityTypeBuilder<MarketPriceSnapshot> b) { Mapping.Base(b,"MarketPriceSnapshots"); b.Property(x=>x.Ean).HasMaxLength(20).IsRequired(); b.Property(x=>x.Rede).HasMaxLength(80).IsRequired(); b.Property(x=>x.NomeProdutoNaRede).HasMaxLength(200); b.Property(x=>x.Preco).HasPrecision(18,2); b.Property(x=>x.TipoPreco).HasConversion<string>().HasMaxLength(30); b.Property(x=>x.Confianca).HasConversion<string>().HasMaxLength(20); b.HasIndex(x=>new{x.Ean,x.Rede}).IsUnique(); } }
