...
[ApiController, Route("api/v1/purchases"), Authorize]
public sealed class PurchasesController(IPurchaseService service) : ControllerBase
{
    [HttpPost, Authorize(Policy="purchases.write")] public async Task<ActionResult<PurchaseResponse>> Create(CreatePurchaseRequest request,CancellationToken ct){var item=await service.CreateAsync(request,ct);return Created($"api/v1/purchases/{item.Id}",item);}
